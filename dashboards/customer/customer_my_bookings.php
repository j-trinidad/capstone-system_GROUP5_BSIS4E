...
$where_clause = "b.customer_id = ? AND b.status IN ('pending', 'in_progress')";

if ($filter === 'pending') {
    $where_clause = "b.customer_id = ? AND b.status = 'pending'";
} elseif ($filter === 'in_progress') {
    $where_clause = "b.customer_id = ? AND b.status = 'in_progress'";
}
...
<title>My Bookings - MotorService</title>
<link href="https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;600;700&display=swap" rel="stylesheet">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
<style>
/* ── ROOT: exactly matches customer_dashboard.php ─────────────────── */
:root {
    --primary:        #1a56db;
    --secondary:      #1e40af;
    --dark-bg:        #f0f4ff;
...
.message.success { background: rgba(5,150,105,0.12); border: 1px solid var(--success); color: var(--success); }
...
.status.pending    { background: linear-gradient(45deg, #f59e0b, #d97706); color: #fff; }
.status.in_progress{ background: linear-gradient(45deg, var(--primary), var(--secondary)); color: #fff; }
.status.on_hold    { background: linear-gradient(45deg, #3b82f6, #2563eb); color: #fff; }
.status.preparing  { background: linear-gradient(45deg, #06b6d4, #0891b2); color: #fff; }
...
.cancel-btn:hover { transform: translateY(-2px); box-shadow: 0 5px 15px rgba(220,38,38,0.35); }
.cancel-btn:disabled { opacity: 0.5; cursor: not-allowed; transform: none; }
...
.receipt-btn {
    ...
    transition: all 0.3s ease;
    background: linear-gradient(135deg, var(--primary), var(--secondary));
    color: #ffffff; display: inline-flex; align-items: center; gap: 8px;
    text-decoration: none; box-shadow: 0 3px 10px rgba(26,86,219,0.2);
}
.receipt-btn:hover { transform: translateY(-2px); box-shadow: 0 5px 15px rgba(26,86,219,0.35); }
...
@media (max-width: 768px) {
    ...
    .booking-summary { flex-direction: column; text-align: center; gap: 10px; }
    .summary-left { flex-direction: column; gap: 8px; }
    .summary-right { min-width: auto; }
    .details-grid { grid-template-columns: 1fr; }
    .filter-buttons { overflow-x: auto; flex-wrap: nowrap; padding-bottom: 5px; }
    .action-buttons { flex-direction: column; }
    .cancel-btn, .receipt-btn { width: 100%; justify-content: center; }
}
</style>
</head>
<body>
...
<main class="main">
    ...
    <div class="filter-buttons">
        <a href="?filter=all"         class="filter-btn <?= $filter==='all'         ? 'active' : '' ?>"><i class="fas fa-list"></i> All</a>
    ...
    <?php else: ?>
            ...
            <div class="booking-summary" onclick="toggleDetails(this)">
                <div class="summary-left">
                    ...
                    <div class="summary-info">
                        ...
                        <p>Scheduled: <?= !empty($b['schedule']) ? date('M d, Y h:i A', strtotime($b['schedule'])) : 'TBA' ?></p>
                    </div>
                </div>
                <div class="summary-right">
                    <div class="total-price">₱<?= number_format($b['total_price'], 2) ?></div>
                    <div class="status <?= strtolower(str_replace(' ','_',$b['status'])) ?>">
            ...
            <div class="booking-details">
                <div class="details-grid">
                    ...
                    <div class="detail-item">
                        ...
                        <p style="text-transform:capitalize;"><?= str_replace('_', ' ', $b['status']) ?></p>
                    </div>
                    <div class="detail-item">
                ...
                <div class="action-buttons">
                    ...
                    <?php endif; ?>
                    <a href="customer_receipt.php?id=<?= $b['id'] ?>" class="receipt-btn">
        ...
        <div class="pagination">
            <?php if ($page > 1): ?>
                <a href="?page=<?= $page-1 ?>&filter=<?= $filter ?>"><i class="fas fa-chevron-left"></i> Prev</a>
            <?php endif; ?>
            <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                <a href="?page=<?= $i ?>&filter=<?= $filter ?>" class="<?= $i==$page ? 'current' : '' ?>"><?= $i ?></a>
            <?php endfor; ?>
            <?php if ($page < $total_pages): ?>
                <a href="?page=<?= $page+1 ?>&filter=<?= $filter ?>">Next <i class="fas fa-chevron-right"></i></a>
            <?php endif; ?>
        </div>
        <?php endif; ?>
    <?php endif; ?>
